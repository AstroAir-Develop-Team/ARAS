...
namespace ARAS
{
    ...
    bool WS::SaveClientInfo(websocketpp::connection_hdl hdl)
    {
        airserver::connection_ptr con = m_server.get_con_from_hdl( hdl );
        std::string time = timestamp();
        std::string name = "client/client_" + time + ".json";
        configor::json j;
        j["Host"] = con->get_host();
        j["Port"] = con->get_port();
        ...
        tmp << str;
        tmp.close();
        std::fstream tmpi(name,std::ios::in);
        if(!tmpi.is_open())
     ...
     * 描述：服务器断开连接时清空数据
     */
    void WS::on_close(websocketpp::connection_hdl hdl)
    {
     ...
     */
    void WS::on_message(websocketpp::connection_hdl hdl,message_ptr msg)
    {
    ...
    void WS::JsonParser(std::string message)
    {
        configor::json j = configor::json::parse(message);
     ...
     */
    void WS::send(std::string message)
    {
     ...
     */
    void WS::run(int port,configor::json config)
    {
